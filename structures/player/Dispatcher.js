...
const nodeStatus = new WebhookClient({ url: 'https://discord.com/api/webhooks/922456267144962098/00DJbXuc8Gv2GRWC8o1bhXScuG3rRonSe8oiBbqZf3KERBPpicJLbQKluVSGz3kwSuIv' });
    ...
    this.player.on("start", async () => {
        ...
        this.client.send(this.textChannelID, {
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(`${this.current.info.title || 'No title available.'}`)
      ...
      if (Date.now() > this.checkTime) {
        ...
        let m = await channel.send({
          embeds: [{
            color: this.client.util.color.primary,
            description: `Enjoying using Noise, give us a vote on top.gg, [Click here](https://top.gg/bot/941681961859158017/vote) to vote`
          }]
        });
